...
class Punctuation:
    ...
    def __init__(self):
        ...
        self.root.title("Punctuation")  # title refers to the name provided to the window. It appears on the top of the
        ...
        with open("PunctuationQuestions.json", "r") as infile:  # opens file, then closes file without needing to type code to do so
        ...
        self.imageRaw = Image.open("Pictures for project/pexels-anna-shvets-3683098.jpg")
        ...
        # anel.grid(row=9)
        self.imgpanel.grid(row = 10, column = 2)
    ...
    def SelectNextQuestion(self):
        ...
        while status:  # while status is True
            ...
            if len(self.QuestionUsed) >= 2:  # if the length of the list of the question that have been used
                # are bigger then 2
                try:
                    StoreResults.storeResults(self.countWrong, self.countCorrect, "Punctuation", self.WrongQuestions,
                                              self.TheirAnswer)
...
class Display():
    def __init__(self):
        QuizType = "Punctuation"  # I am defining the specific quiz type that this module refers to
