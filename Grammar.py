...
class Grammar:
    ...
    def __init__(self):
        pygame.mixer.init()
        pygame.mixer.music.load("Music For Project/a-jazz-piano-110481.mp3")
        ...
        self.root.geometry("700x515")  # This method is used to set the dimensions of the Tkinter window and is used to
        # set the position of the main window on the user’s desktop.
        self.root.title("Grammar")  # title refers to the name provided to the window. It appears on the top of the
    ...
    def SelectNextQuestion(self):
        ...
        while status:  # while status is True
            ...
            if len(self.QuestionUsed) >= 2:  # if the length of the list of the question that have been used
                # are bigger then 2
                try:
                    StoreResults.storeResults(self.countWrong, self.countCorrect, "Grammar", self.WrongQuestions,
                                              self.TheirAnswer)
