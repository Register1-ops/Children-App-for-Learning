...
class Level1Colours():
    def __init__(self):
        ...
        # self.ButtonFrame.pack(anchor = "e", fill = tk.X)

        self.InFrame = tk.Frame(self.root)
        self.InFrame["bg"] = "#faab34"
        self.InFrame.pack()
        ...
        self.root.geometry(
            "600x350")  # This method is used to set the dimensions of the Tkinter window and is used to
        # set the position of the main window on the user’s desktop.
        self.root.title(
            "Level 1 Colours")  # title refers to the name provided to the window. It appears on the top of the
    ...
    def PlaceButtons(self):
        ...
        self.drawShape(self.CurrentQuestion["colour"])
        return
    ...
    def SelectNextQuestion(self):
        ...
        while status:  # while status is True
            ...
            if len(self.QuestionUsed) >= 2:  # if the length of the list of the question that have been used
                # are bigger then 2
                try:
                    StoreResults.storeResults(self.countWrong, self.countCorrect, self.QuizType,
                                              self.WrongQuestions,
                                              self.TheirAnswer)
                    # calls the function storeResults and passes through the given parameters
                    self.root.destroy()  # the page is destroyed
                    pygame.mixer.music.stop()
                    Display(self.QuizType)  # calls the functon display
