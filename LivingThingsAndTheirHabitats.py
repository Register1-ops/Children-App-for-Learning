...
class LivingThingsAndTheirHabitat:
    def __init__(self):
        pygame.mixer.init()
        ...
        self.root.geometry("615x515")  # This method is used to set the dimensions of the Tkinter window and is used to
        # set the position of the main window on the user’s desktop.
        self.root.title("Living Things and Their Habitats")  # title refers to the name provided to the window. It appears on the top of the
        ...
        with open("LivingThingsAndTheirHabitatsQuestions.json",
    ...
    def SelectNextQuestion(self):
        ...
        while status:  # while status is True
            ...
            if len(self.QuestionUsed) >= 2:  # if the length of the list of the question that have been used
                # are bigger then 2
                try:
                    StoreResults.storeResults(self.countWrong, self.countCorrect, "LivingThingsAndTheirHabitats", self.WrongQuestions,
                                              self.TheirAnswer)
...
class Display():
    def __init__(self):
        QuizType = "LivingThingsAndTheirHabitats"
        DisplayResults.Display(QuizType)

if __name__ == "__main__":
    LivingThingsAndTheirHabitat()
