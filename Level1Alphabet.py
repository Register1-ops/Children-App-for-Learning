...
class Level1Alphabet:
    ...
    def __init__(self):
        ...
        self.root.geometry("1000x500")  # This method is used to set the dimensions of the Tkinter window and is used to
        # set the position of the main window on the user’s desktop.
        self.root.title("Level 1 Alphabet")  # title refers to the name provided to the window. It appears on the top of the
        ...
        with open("Level1AlphabetQuestions.json",
                  "r") as infile:  # opens file, then closes file without needing to type code to do so
            self.questions = json.load(infile)

        self.width = 300
        self.height = 200
        self.imageRaw = Image.open("Pictures for project/pexels-pixabay-278887.jpg")
    ...
    def SelectNextQuestion(self):
        ...
        while status:  # while status is True
            ...
            if len(self.QuestionUsed) >= 2:  # if the length of the list of the question that have been used
                # are bigger then 2
                try:
                    StoreResults.storeResults(self.countWrong, self.countCorrect, "Level1Alphabet", self.WrongQuestions,
                                              self.TheirAnswer)
