...
class RhymingWords:
    ...
    def __init__(self):
        ...
        self.root.geometry("625x515")  # This method is used to set the dimensions of the Tkinter window and is used to
        # set the position of the main window on the user’s desktop.
        self.root.title("Rhyming Words")  # title refers to the name provided to the window. It appears on the top of the
        ...
        with open("RhymingWordsQuestions.json", "r") as infile: # opens file, then closes file without needing to type code to do so
        ...
        # anel.grid(row=9)
        self.imgpanel.place(x=100, y=300)
    ...
    def SelectNextQuestion(self):
        ...
        while status: # while status is True
            ...
            if len(self.QuestionUsed) >= 2:   # if the length of the list of the question that have been used
                # are bigger then 2
                try:
                    StoreResults.storeResults(self.countWrong, self.countCorrect, "RhymingWords", self.WrongQuestions, self.TheirAnswer)
...
class Display():
    def __init__(self):
        QuizType = "RhymingWords"  # I am defining the specific quiz type that this module refers to
        DisplayResults.Display(QuizType)
        # This part calls the Display class inside DisplayResults module, it also passes the Quiz type into the
